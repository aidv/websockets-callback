class WebSockets_Callback{
        ...
        this.ws.on('connection', function(conn) {
            ...
            conn.on('message', function(message){
                ...
                if (json.puid != undefined){
                    ...
                    if (t.options.onUnexpectedMessage != undefined) t.options.onUnexpectedMessage(conn, event)
                }
            });
    ...
    send(message, onResponse = undefined, onProgress = undefined, conn = undefined){
        ...
        var t_ = (conn != undefined ? conn : this.ws)
        t_.send(JSON.stringify(message));
    }

    simple(msg, onResponse = undefined, t = undefined){
        var t_ = (t != undefined ? t.ws : this.ws)
        t_.send({cmd: msg}, onResponse);
    }
